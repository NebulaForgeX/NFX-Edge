// Clear (truncate) all table data in Edge product schemas — dev only.
//
// It TRUNCATEs every table in: sites
// (RESTART IDENTITY CASCADE). Table structures are kept; only rows are removed.
//
// Usage (from NFX-Edge repo root):
//
...
import (
	...
	sitesconfig "nfxedge/modules/sites/config"
	"nfxedge/pkgs/env"
	"nfxedge/pkgs/logx"
	"nfxedge/pkgs/postgresqlx"
)

var schemas = []string{"sites"}
...
func main() {
	...
	if _, err := os.Stat(".env"); os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, "run from NFX-Edge repo root (missing .env in cwd)\n")
		os.Exit(1)
	}

	cfg, err := sitesconfig.Load(ctx, clearEnv)
	if err != nil {
