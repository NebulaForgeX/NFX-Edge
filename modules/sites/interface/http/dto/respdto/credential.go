...
import (
	...
	credResults "nfxedge/modules/sites/application/credential/results"
)
...
func CredentialListToDTO(rows []credResults.CredentialRO) []CredentialDTO {
	out := make([]CredentialDTO, 0, len(rows))
	for i := range rows {
		if dto := CredentialROToDTO(&rows[i]); dto != nil {
			out = append(out, *dto)
		}
	}
	return out
