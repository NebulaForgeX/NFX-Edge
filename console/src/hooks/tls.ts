...
export const useSearchCertificate = () => {
  const tls = useTlsRepository();
  return useMutation({
    mutationFn: (request: SearchCertificateRequest) => tls.SearchCertificate(request),
    onError: (error: AxiosError) => showError(getApiErrorMessage(error, "[useSearchCertificate]")),
  });
};
