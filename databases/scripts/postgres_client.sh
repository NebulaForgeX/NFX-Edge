...
nfxedge_effective_pgsslmode() {
  local s
  s=$(nfxedge_normalize_pgsslmode)
  if [[ -n "$s" ]]; then
    printf '%s' "$s"
    return
  fi
...
nfxedge_psql_admin() {
  if nfxedge_postgres_use_docker; then
    ...
    nfxedge_docker exec "${POSTGRES_CONTAINER_NAME}" psql -U "${POSTGRES_USER}" -d postgres "$@"
  else
    nfxedge_ensure_psql_cli || return 1
    export PGPASSWORD="${POSTGRES_PASSWORD}"
    export PGSSLMODE="$(nfxedge_effective_pgsslmode)"
