...
func main() {
	...
	if err := logx.Init(cfg.Logger, "sites-base-service", env.Env(*envStr)); err != nil {
		log.Fatalf("logger init failed: %v", err)
	}
	defer logx.Sync()

	otelShutdown, err := otelx.Init(ctx, cfg.OTEL, "sites", env.Env(*envStr))
	if err != nil {
